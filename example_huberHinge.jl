...
objFunc(z,w;konst=nothing) = hhObjLinear(z,w,y,k=konst)
fPrimeFunc(z,w;konst=nothing) = hhFPrimeLinear(z,w,y,k=konst)
gradFunc(z,w,X;konst=nothing) = hhGradLinear(z,w,X,y,k=konst)

# function and gradient wrt step size for line search that calls minFunc
funObjForT(t,D,w,X) = hhObjAndGrad(t,D,w,X,y)

# function and gradient for minFuncNonOpt
funObjForNonOpt(w,X) = hhObjAndGrad(w,X,y)

# function and no gradient evaluation for minFuncNonOpt/ lsArmijoNonOpt
funObjNoGradForNonOpt(w,X) = hhObjAndNoGrad(w,X,y)
...
for a=1:nDatasets
    ...
    (m,n) = size(X)
    #w0 = ones(n,1) ./sqrt(n) # initial solution
