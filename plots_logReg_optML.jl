...
# List of datasets
datasetNames=["sido","spam"]
nDatasets=size(datasetNames,1)
datasets = [] # holds configuration for each dataset (such as max number of iterations to fit)

push!(datasets,(label="sido",xFile="./data/classificationBinary/sido0_train.mat",xLabel="X",
    yFile="./data/classificationBinary/sido0_train.targets",yLabel="",maxPasses=250,
    maxLsIter=25,maxTimeInSec=600,addBias=true,yLabelZeroOne=false,transposeX=false))
push!(datasets,(label="spam",xFile="./data/classificationBinary/trec2005.mat",xLabel="A",
    yFile="",yLabel="y",maxPasses=150,maxLsIter=25,maxTimeInSec=600,addBias=false,
    yLabelZeroOne=true,transposeX=true))
...
for a=1:nDatasets
    ...
    data = matread(datasets[a].xFile)
    global X = (data[datasets[a].xLabel])
    if datasets[a].transposeX
        global X = X'
    end    
    
    # get labels
    if datasets[a].yFile!=""
        global y = readdlm(datasets[a].yFile)
    else
        global y = data[datasets[a].yLabel]
    end
    if datasets[a].yLabelZeroOne
        replace!(y,0.0=>-1.0)
    end
    global y = y[:,1]
    ...
    # Add bias variable, initialize 
    if datasets[a].addBias
        global X = [ones(m,1) X]
        n += 1
    end
    ...
    push!(subplotByIter[methodLabel],plot(x,fValuesIterPlot[methodLabel],label=fValuesIterPlotLabels[methodLabel],
        title=methodLabel,titlefont=font(8),xlabel="iterations",ylabel="optimality gap",#linewidth=2,
